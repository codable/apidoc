		...
		for(var j = 0; j < elements.length; j += 1)
		{
			...
			if(self.parsers[element.name])
			{
				...
				try {
					...
					if (blockData.global.paramTitle)
						defaultGroup = blockData.global.paramTitle[blockData.global.paramTitle.length - 1].group;
					result = self.parsers[element.name].parse(element.content, element.source, defaultGroup);
				}
				catch(e)
				{
...
Parser.prototype._findBlockWithApiGetIndex = function(elements)
{
	...
	for(var i = 0; i < elements.length; i += 1)
	{
		...
		{
			if(this.parsers[elements[i][j].name])
			{
				foundIndex = j;
			}
			if(elements[i][j].name === "ignore")
			{
				foundIndex = -1;
				break;
			}
